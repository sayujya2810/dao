...
const CreatePost = () => {
  ...
  useEffect(() => {
    setEncryptedMessage(CryptoJS.AES.encrypt(message,process.env.REACT_APP_ENCRYPTION_KEY))
  },[message,setMessage])

  useEffect(() => {
    setEncryptedCategory(CryptoJS.AES.encrypt(category,process.env.REACT_APP_ENCRYPTION_KEY))
  ...
  //   useEffect(() => {
  //   setDecryptedMessage(CryptoJS.AES.decrypt(encryptedMessage,process.env.REACT_APP_ENCRYPTION_KEY))
  ...
  const createPost = async () => {
    ...
    const tx = await contract.createPost(encryptedCategory.toString(),duration,encryptedMessage.toString()); //edit categoty is string, duration(in seconds)
  ...
  return (
    <div id='main'>
      <form id='container' style={{marginTop:"2rem"}} onSubmit={handleCreateSubmit}>
          ...
          <input min={12} onChange={(e) => setHours(e.target.value)} type="number" placeholder='Hours'/>
