...
const MyPosts = (props) => {
  ...
  const postsOfOwner = async () => {
    ...
    const tx = await contract.postsOfOwner(address); //edit user address -->  done ... keep this wallet address for testing ... then change it to dynamic one the  "address" variable
    setPostIdHex(tx)
    // console.log(address)
    console.log(tx)
};
